      ...
      before(app) {
        // app.get('请求地址',(req,res)=>{
          ...
          let userpoor = [
            ...
            {username:'zhangq', password: '123456'}
          ]
          //注册接口
          app.get('/api/register', (req,res) => {
            const {username, password} = req.query;
            const userlength = userpoor.filter(i => i.username === username).length;
            if(userlength > 0) {
              res.json({
                success: false,
                message: '用户名已存在'
          ...
          app.get('/api/login', (req, res) => {
            const {username, password} = req.query;
            if(username=='xiaod' && password=='123456' || username=='zhangq' && password=='123456') {
              res.json({
                code: 0,
                message: '登录成功',
           ...
           app.get('/api/banner',(req,res)=>{
            res.json({
                data:[  {
                    ...
                    image: 'https://file.xdclass.net/video/2022/banner/7-VIP.png'
                  },
                  {
                    url: 'https://m.xdclass.net',
                    image: 'https://file.xdclass.net/video/2022/banner/7-XD.png'
                  },
                  {
                    url: 'https://m.xdclass.net',
                    image: 'https://file.xdclass.net/video/2022/22-aliyun/7.png'
                  }]
            })
        })
        //滚动分类接口
